...
class NetworkStatusLiveData(context: Context) : LiveData<Boolean>() {
    ...
    private fun createNetworkCallback() = object : ConnectivityManager.NetworkCallback() {
        ...
        override fun onAvailable(network: Network) {
            Log.d("NetworkStatus", "onAvailable: ${network}")
            val networkCapabilities = cm.getNetworkCapabilities(network)
            val hasInternetCapability =
                networkCapabilities?.hasCapability(NET_CAPABILITY_INTERNET)
            Log.d("NetworkStatus", "onAvailable: ${network}, $hasInternetCapability")
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    if (hasInternet) {
                        withContext(Dispatchers.Main) {
                            Log.d("NetworkStatus", "onAvailable: adding network. ${network}")
        ...
        override fun onLost(network: Network) {
            Log.d("NetworkStatus", "onLost: ${network}")
